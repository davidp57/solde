...
# Mapping from BankTransactionCategory to TriggerType for direct reconciliation.
# Categories that go through their own service (payment, deposit, salary) are absent:
# their accounting entries are already generated by those services.
_BANK_CATEGORY_TRIGGER: dict[str, TriggerType] = {
    "bank_fee": TriggerType.BANK_FEES,
    "social_charge": TriggerType.BANK_SOCIAL_CHARGES,
    "grant": TriggerType.SUBSIDY_RECEIVED,
    # INTERNAL_TRANSFER is split by direction — handled explicitly below.
}


async def generate_entries_for_bank_transaction(
    db: AsyncSession,
    tx: object,
) -> list[AccountingEntry]:
    """Generate accounting entries when a bank transaction is reconciled.

    The *tx* parameter is a BankTransaction instance (typed as object to avoid
    a circular import between accounting_engine and bank models at call time).
    ...
    if category == BankTransactionCategory.INTERNAL_TRANSFER:
        ...
        trigger: TriggerType = (
            ...
            if tx.amount > 0
            else TriggerType.BANK_INTERNAL_TRANSFER_TO_SAVINGS
        )
    else:
        resolved = _BANK_CATEGORY_TRIGGER.get(category)
        if resolved is None:
            return []
        trigger = resolved
