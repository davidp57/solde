...
import { deleteInvoiceApi, listInvoicesWithCountApi } from '../../api/invoices'
...
describe('SupplierInvoicesView — preview dialog bottom navigation', () => {
  ...
  it('shows a saved invoice without asking for the list again', async () => {
    ...
    const created = { ...invoiceFixture, id: 999, number: 'FF-2026-0999' }
    const vm = wrapper.vm as unknown as { onSaved: (invoice: typeof invoiceFixture) => void }
    vm.onSaved(created)
    await flushView()

    expect(wrapper.text()).toContain('FF-2026-0999')
