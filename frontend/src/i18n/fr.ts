export default {
  ...
  accounting: {
    ...
    journal: {
      ...
      workspace_subtitle: 'Une ligne par operation, puis le detail des ecritures au clic pour garder une lecture fiable et compacte.',
    ...
    fiscalYear: {
      ...
      close_administrative_confirm: "Marquer l'exercice {name} comme clôturé sans générer de nouvelles écritures de clôture ou de report à nouveau ?",
  ...
  import: {
    ...
    guidance_common_exercise: 'L’import ne crée pas automatiquement les exercices comptables. Créez d’abord chaque exercice couvrant les dates à importer.',
    guidance_common_seed_accounts: 'L’import ne crée pas automatiquement le plan comptable. Préparez vos comptes avant de contrôler le résultat.',
    guidance_common_seed_rules: 'L’import ne crée pas automatiquement les règles comptables. Configurez-les séparément pour la génération d’écritures depuis la gestion courante.',
    guidance_gestion_scope: 'Gestion importe les contacts, factures, paiements et mouvements de trésorerie du classeur historique.',
    guidance_gestion_fiscal_year: 'Les écritures générées par la gestion sont rattachées à l’exercice couvrant leur date quand il existe ; sinon elles restent sans exercice.',
    guidance_gestion_supplier: 'Les références FF-... détectées dans Banque et Caisse peuvent créer des factures fournisseurs historiques et leur règlement.',
    guidance_compta_scope: 'Comptabilité importe la feuille Journal comme écritures manuelles supplémentaires.',
    guidance_compta_coexistence: 'Si des écritures issues de la gestion existent déjà, les doublons exacts du journal sont ignorés et seules les écritures nouvelles sont ajoutées.',
    guidance_compta_chart: 'Le plan comptable reste à préparer pour la lecture et le contrôle, même si l’import comptabilité ne génère pas ses lignes à partir des règles comptables.',
    test_shortcuts_title: 'Raccourcis de test temporaires',
    test_shortcuts_subtitle: 'Lancez directement les imports historiques configurés côté serveur, dans l’ordre recommandé, sans prévisualisation.',
    test_shortcuts_hint: 'Ces boutons sont destinés aux tests locaux. Ils restent désactivés tant que les fichiers attendus ne sont pas trouvés.',
    ...
    result_persistent_hint: '{count} élément(s) créé(s), {ignored} ligne(s) ignorée(s) et {blocked} ligne(s) bloquantes. Le détail complet reste affiché ci-dessous.',
