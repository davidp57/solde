...
async def test_list_transactions_unreconciled_only(db_session: AsyncSession) -> None:
    t1 = await bank_service.add_transaction(
        db_session, BankTransactionCreate(date=date(2024, 1, 1), amount=Decimal("100.00"))
    )
    await bank_service.add_transaction(
        db_session, BankTransactionCreate(date=date(2024, 1, 2), amount=Decimal("200.00"))
    )
...
async def test_get_bank_balance(db_session: AsyncSession) -> None:
    await bank_service.add_transaction(
        db_session, BankTransactionCreate(date=date(2024, 1, 1), amount=Decimal("500.00"))
    )
    await bank_service.add_transaction(
        db_session, BankTransactionCreate(date=date(2024, 1, 2), amount=Decimal("-100.00"))
    )
...
@pytest.mark.asyncio
async def test_create_deposit_marks_payments_deposited(db_session: AsyncSession) -> None:
