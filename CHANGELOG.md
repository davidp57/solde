...
### Corrigé
- **TEC-254** — **Le compteur « opérations non rapprochées » de la séance annonçait 212 là où il en restait 3 à traiter.** Il comptait toute ligne non rapprochée du journal, sans distinguer trois natures très différentes : les **209 opérations reprises de l'historique** lors de la mise en service (source `import`, datées de 2024-2025) et le solde d'ouverture, qui n'ont jamais eu vocation à être rapprochés ; les **remises enregistrées en attendant leur relevé**, que seul un import futur peut solder ; et les vraies opérations de relevé restant à traiter. Le compteur ne retient désormais que ces dernières, datées au plus tard à la fin du mois traité — un relevé exporté en « toutes les opérations disponibles » apporte aussi des lignes du mois suivant, qui sont le travail de la séance suivante. Les remises en attente sont annoncées **séparément**, et seulement s'il y en a.

### Modifié
- **BIZ-257** — **La séance de comptabilité s'ouvre en panneau latéral et sa liste est réorganisée**, après les premiers retours d'usage. Le panneau s'ouvre à droite et **ne bloque plus l'application** : il peut rester ouvert pendant qu'on travaille, ce que la fenêtre modale interdisait — il fallait la fermer pour agir, la rouvrir pour cocher. Côté contenu : « Vérifier le récapitulatif mensuel » retirée (redondante avec la saisie des fiches) ; **encodage des règlements de factures fournisseurs payés en espèces** ajouté, qui manquait ; le bloc « Caisse et remises » scindé en **5. Caisse** (les mouvements du tiroir, puis le comptage — compter avant d'avoir tout saisi comparerait le tiroir à un total périmé) et **6. Bordereaux de remise**, dont le titre indique qu'ils se préparent dans l'écran Banque et non dans l'écran Caisse. Toujours dix-sept étapes.

### Ajouté
- **BIZ-260/261** — **Défaire le rapprochement d'une opération bancaire.** Rapprocher était sans retour : `reconciled` passait à `True` en trois endroits du service et rien ne le remettait jamais à `False`. Or ce drapeau est le verrou de tout le reste — il ferme la suppression, l'édition de la date, du montant, du compte et de la catégorie, ainsi que le rattachement d'un règlement. Une opération rapprochée par erreur était donc **définitivement figée**, y compris quand son rapprochement n'avait produit aucune écriture (cas d'une catégorie « Sans écriture », le plus courant). C'est ce qui est arrivé au doublon du compte d'épargne corrigé en BIZ-258/259 : la ligne en trop était rapprochée, il a fallu supprimer l'autre — celle qui portait la référence de la banque. Une action **« Défaire le rapprochement »** (trésorier et administrateur) remet l'opération « à rapprocher » et supprime les écritures `bank_transaction:{id}` que le rapprochement avait générées. Elle ne couvre que le rapprochement direct : un rapprochement **porté par un règlement** ou **par un bordereau de remise** est refusé avec un motif qui nomme le bon chemin, plutôt que défait à moitié — défaire le drapeau seul laisserait le règlement ou le bordereau pointer une ligne qui ne les reconnaît plus. Un exercice clôturé est refusé également : cela relève de la contre-passation. Rien ne reliant une ligne bancaire à un bordereau dans le schéma, l'appartenance est reconnue par les trois marques que les chemins de remise laissent : la référence `DEP-CHQ-…`, l'étiquette « Bordereau #N », et surtout la **description** — seule à survivre à une fusion, où la ligne prend la référence du relevé et où l'étiquette peut n'avoir jamais été renseignée. Cette description est désormais construite en un seul endroit, pour qu'elle et la lecture qui en est faite ne divergent pas.
- **BIZ-258/259** — **Les doublons d'import bancaire sont signalés, et une ligne importée en double peut enfin être supprimée.** Cas réel : le solde du compte d'épargne affiché dans l'écran Banque annonçait 39 820,46 € là où la banque et la balance comptable disaient 42 820,46 €. Le virement interne du 01/05/2026 y figurait **deux fois** — une saisie faite à la main le 04/05, puis la même opération ramenée par l'import OFX du 30/06 sous le libellé de la banque. Deux défauts s'étaient additionnés. D'une part, la détection des doublons à l'import ne comparait que les **références bancaires** : une opération saisie à la main n'en porte aucune, donc rien ne pouvait rapprocher les deux lignes. D'autre part, **aucune des deux ne pouvait être supprimée** : la saisie manuelle était rapprochée (et rien ne permet de dé-rapprocher), la ligne importée ne l'était pas mais la suppression était réservée aux opérations manuelles — impasse complète, dont la seule sortie était une intervention directe en base. Désormais : (1) un import qui ramène une opération ressemblant à une opération déjà enregistrée — **même compte, même montant, à trois jours près** — l'importe quand même mais **le dit**, en gardant le dialogue d'import ouvert sur les deux lignes côte à côte, avec un bouton de suppression sur chacune ; (2) une opération issue d'un import de relevé (OFX, CSV, QIF) et non rapprochée peut être supprimée depuis l'écran Banque. Le choix de **signaler plutôt qu'écarter** est délibéré : écarter automatiquement aurait supprimé la ligne qui porte la référence de la banque au profit d'une saisie approximative, et surtout aurait masqué l'écart au lieu de le montrer. Les lignes issues d'un **import Excel** restent verrouillées (l'annulation d'import a besoin d'elles), les opérations rapprochées aussi, et les opérations importées ne sont toujours pas modifiables. *(La comptabilité n'avait jamais été fausse : un virement interne ne produit ses écritures que depuis le côté compte courant, donc une ligne d'épargne en trop n'a aucun effet comptable — ce qui explique aussi que l'écart ait pu vivre deux mois sans se voir.)*
- **BIZ-254/255/256** — **Séance de comptabilité : la checklist mensuelle.** La tenue du mois enchaîne une vingtaine de gestes répartis sur quatre écrans, entrecoupés de passages sur la plateforme de paie et sur le site de la banque ; le manuel les décrivait en quatre parcours séparés, sans dire dans quel ordre les enchaîner ni comment gérer ces allers-retours. Résultat : l'ordre se redécouvrait chaque mois, rien ne retenait où l'on en était après une interruption, et les étapes de contrôle — comptage de caisse, comparaison du solde au relevé — sautaient en premier. Un bouton dans le bandeau, présent sur tous les écrans, ouvre désormais une liste ordonnée de dix-sept étapes en six blocs, cochée au fur et à mesure ; l'état vit en base, donc il suit d'un poste à l'autre et forme un historique. Principe d'ordonnancement retenu avec l'utilisateur : **un seul passage par destination externe**, tout ce qui le conditionne étant fait avant — d'où un unique passage à la banque en ligne, pour les virements *et* le téléchargement du relevé. Les étapes qui se font hors de l'application sont signalées comme telles. Six étapes affichent un fait observé (« Dernier import le 01/09, 12 opérations », « Chèques à remettre : 2, soit 234,00 € ») mais **ne cochent jamais rien** : une checklist sert à vérifier, pas à constater. Le mois traité est proposé selon la date — le mois précédent avant le 15, le mois en cours ensuite — en sautant les mois déjà tenus. Une seule séance ouverte à la fois ; la clôture affiche ce qui n'a pas été coché avant de demander confirmation, une séance incomplète peut être close, et ce qui reste est signalé « En retard » sur la séance suivante. Réservé au trésorier et à l'administrateur.
- **BIZ-253** — **Confirmer un bordereau ne crée plus de doublon quand le relevé est déjà là.** L'application savait déjà, à l'import, reconnaître une remise déjà enregistrée (BIZ-227) ; elle ne savait pas regarder dans l'autre sens. Confirmer un bordereau dont le mouvement figurait déjà au relevé — le cas courant d'une confirmation faite quelques jours après le crédit — ajoutait une seconde opération, gonflait le solde affiché du montant de la remise, et laissait l'utilisateur résorber le doublon à la main (cas réel : remise de chèques de 111 €, solde passé de 1 191,72 € à 1 302,72 € le temps de l'opération). À la confirmation, l'application cherche désormais au relevé une opération non rapprochée du même montant et de la **même** nature de remise, dans une fenêtre de ± 10 jours autour de la date du bordereau : s'il y en a exactement une, elle la reprend — libellé du bordereau, date, référence et source de la banque conservées, ligne rapprochée — au lieu d'en créer une seconde. Un dépôt d'espèces n'est jamais repris par un bordereau de chèques du même montant, et si plusieurs opérations pourraient convenir, aucune n'est touchée.
- **BIZ-252** — **Le rattachement d'une remise indique le bordereau qui attend d'être confirmé.** Un bordereau non confirmé ne crée aucune opération bancaire : il n'y a donc rien à lui rattacher, et le dialogue se contentait de répondre qu'aucun bordereau ne correspondait — impasse, alors que le bon bordereau existait à un clic de confirmation près (cas réel : une remise de chèques de 111 € du 04/08). Quand aucune ligne ne peut être rattachée, le dialogue cherche désormais parmi les bordereaux **en attente de confirmation** celui du même type et du même montant, et le nomme avec sa date en disant quoi faire.
- **BIZ-251** — **Rattacher une ligne de relevé à un bordereau déjà enregistré.** Quand le rapprochement automatique d'une remise n'a pas eu lieu, le journal bancaire porte deux lignes pour un seul mouvement et la seule issue était de supprimer la provisoire à la main, puis de rapprocher l'autre. Une action **« Rattacher à un bordereau enregistré »**, proposée sur une ligne de relevé non rapprochée reconnue comme remise, liste les bordereaux du même montant en attente — les plus proches en date d'abord — et fusionne les deux lignes : celle du bordereau est conservée (sa description le nomme, là où le libellé bancaire « REM CHQ REF05001A05 » n'apprend rien), elle prend la date, la référence et la source du relevé, et la ligne importée disparaît. Même résultat que la fusion à l'import, mais c'est l'utilisateur qui désigne : le rattachement exige un montant identique et refuse une ligne déjà rapprochée, une ligne saisie à la main ou une opération qui n'est pas une remise.
- **BIZ-250** — **Le montant d'un règlement en espèces n'est plus pré-rempli.** Les dialogues de règlement (fiche facture et facture rapide) proposaient d'office le solde restant dû : enregistrer un montant que personne n'a compté ne demandait qu'un clic, et la seule validation existante refusait un montant *supérieur* au solde — un montant **égal** passait sans un mot. C'est ainsi qu'une facture émise à tort à 310 € a été soldée par un règlement de 310 € alors que 270 € avaient été remis, laissant la caisse théorique en écart de 40 € jusqu'au comptage suivant. Les espèces sont le seul mode sans source externe pour contredire la saisie — un chèque porte son montant, un virement est arbitré par le relevé. Le champ se vide donc au passage en espèces et doit être saisi ; un bouton **« Reporter le solde dû (X €) »** rétablit le raccourci en tant que choix explicite, et l'effet sur la caisse (`solde avant → solde après`) est affiché sous le champ, dans le bon sens selon qu'il s'agit d'un encaissement client ou d'un règlement fournisseur. Le pré-remplissage du mode chèque est inchangé. *(Le garde-fou réduit le risque sans le supprimer : rien n'empêche de taper un montant sans avoir compté — il supprime le cas où l'erreur ne demande aucun geste.)*
- **BIZ-247** — **Aperçu des documents dans l'application**, sans téléchargement préalable : PDF via une visionneuse intégrée, images, Markdown mis en forme (titres, listes, tableaux) et texte brut (CSV, JSON, XML). Le Markdown passe par l'assainisseur déjà utilisé pour l'aide — un fichier déposé pouvant contenir n'importe quoi, le HTML et les scripts qu'il porterait sont retirés. Les formats que le navigateur ne sait pas rendre (Word, Excel, archives) n'affichent pas de bouton d'aperçu plutôt qu'un cadre vide, et le contenu n'est même pas récupéré. Sur mobile, un PDF s'ouvre dans un onglet.
- **BIZ-240/241/242** — **Espace de documents** (Accueil → Documents). Un dépôt libre pour les pièces qui n'appartiennent à aucun objet métier : statuts, procès-verbaux, attestations, états financiers signés. Chaque document porte un titre, une date de dépôt, un exercice **facultatif**, des étiquettes libres et des notes. Filtres combinables par exercice (dont « Sans exercice »), par étiquette et par recherche textuelle. Lecture et téléchargement ouverts à tous les rôles ; dépôt, modification et suppression à partir du rôle gestionnaire. Fichiers stockés dans `data/documents/`, la base ne portant que les métadonnées ; formats vérifiés aux octets d'en-tête et non à l'extension, 20 Mo par fichier ; la suppression retire le fichier du disque.
- **BIZ-236** — **Export PDF du bilan et du compte de résultat**, pour l'archivage comptable : document A4 mis en page avec l'en-tête de l'association, la période de l'exercice, son statut (clôturé / en cours), les deux sections détaillées et leurs totaux. Le fichier prend le nom de l'exercice (`bilan_2025.pdf`). Les exports CSV et Excel restent disponibles.
- **BIZ-234** — **Bascule « Tout l'historique » dans l'écran Banque**, comme sur la Caisse, les Factures et les Paiements. Les opérations et les remises étaient bornées à l'exercice sélectionné, sans moyen de voir au-delà : un relevé importé contenant des écritures postérieures à la fin de l'exercice — cas classique juste après une clôture, ou avant que l'exercice suivant n'existe — restait totalement invisible.

### Corrigé
- **TEC-254** — **Le test d'exposition de Swagger dépendait de la configuration locale du poste.** `test_swagger_disabled_in_production` construisait ses `Settings` sans figer `swagger_enabled`, que pydantic-settings allait donc chercher dans le `.env` du dépôt. Un développeur ayant `SWAGGER_ENABLED=true` en local — configuration légitime, c'est l'intérêt du drapeau — voyait la suite échouer sur un test sans rapport avec son travail, alors que la CI restait verte faute de `.env` : le pire des deux mondes, un échec qui n'apprend rien et ne se reproduit pas là où on le regarde. Les `Settings` de ce test sont désormais construites avec `_env_file=None` et les deux drapeaux qui décident de l'exposition sont passés explicitement. Un cas manquait par ailleurs à la couverture : `SWAGGER_ENABLED` ouvre la documentation **à lui seul**, hors mode debug (`swagger_enabled or debug`), ce qu'aucun test ne vérifiait.
- **TEC-253** — **Une remise fusionnée restait « à rapprocher ».** Après fusion, la ligne *est* le bordereau : la correspondance est établie par construction, et réclamer un clic de plus la laissait dans la file des opérations à traiter sans rien apporter. Les deux chemins de fusion — automatique à l'import et manuel depuis la ligne du relevé — marquent désormais la ligne rapprochée et renseignent sa référence comptable (« Bordereau #8 »), jusque-là vide. Aucune écriture supplémentaire n'est générée : les catégories de remise sont volontairement absentes des déclencheurs comptables, leurs écritures étant produites à la confirmation du bordereau. Si le bordereau porte une référence bancaire saisie à la main, la colonne reste vide plutôt que d'afficher un numéro deviné.
- **TEC-252** — **La demande de confirmation d'un bordereau restait affichée après validation.** Deux boîtes de confirmation étaient montées en même temps — celle de l'écran Banque et une seconde à l'intérieur du dialogue d'actions du bordereau. Le service de confirmation de PrimeVue les ouvre **toutes** ; valider n'en referme qu'une, l'autre restait à l'écran avec son bouton toujours actif. Le bordereau était pourtant bien confirmé : re-valider rejouait l'appel et affichait « Cannot update a confirmed deposit », donnant à croire que la confirmation avait échoué. La boîte appartient désormais à la vue seule ; l'écran d'accueil, qui affiche aussi les bordereaux en attente, en monte une à son tour.
- **TEC-250** — **Une remise confirmée en retard laissait un doublon dans le journal bancaire.** La ligne créée à la confirmation d'un bordereau était datée du **jour du clic** (`date.today()`), pas du jour où la remise avait été portée à la banque. Or la confirmation se fait quand on y repense : cas réel, un bordereau du 03/08 crédité par la banque le 04/08 mais confirmé le 09/08 a produit une ligne au 09/08 — cinq jours plus loin que la fenêtre de rapprochement, donc deux opérations pour un seul mouvement, à nettoyer à la main. La ligne bancaire **et** la sortie de caisse portent désormais la date du bordereau, celle que les écritures comptables du même bordereau utilisaient déjà ; les trois dates concordent enfin. La date de confirmation reste enregistrée sur le bordereau lui-même.
- **TEC-251** — **La fenêtre de rapprochement d'une remise passe de ± 3 à ± 10 jours.** La date d'un bordereau reste déclarative — il peut être préparé quelques jours avant d'être porté à la banque — et trois jours ne laissaient aucune marge une fois la cause principale corrigée (TEC-250). Le garde-fou est inchangé : si **plusieurs** remises du même montant peuvent correspondre, l'application ne choisit toujours pas à votre place et importe les deux lignes.
- **TEC-249** — **La sauvegarde automatique vers OneDrive échouait dès que le dossier distant des PDF dépassait une page de résultats.** Avant d'envoyer les PDF, l'application liste ce qui est déjà présent sur OneDrive pour ne transférer que le manquant. Ce listing désignait le dossier **par son chemin** (`root:/backups/pdfs:/children`) ; passé 200 fichiers, Microsoft Graph répond par un lien de page suivante qui mélange cette écriture par chemin et un jeton de pagination, puis **rejette son propre lien avec une erreur 400** (défaut connu de l'API OneDrive, [onedrive-api-docs#620](https://github.com/OneDrive/onedrive-api-docs/issues/620) et [#774](https://github.com/OneDrive/onedrive-api-docs/issues/774)). Conséquence : l'exception survenait **avant tout envoi**, donc plus aucun PDF n'était copié sur OneDrive depuis que le seuil avait été franchi — la base de données, elle, continuait de partir normalement, d'où un run marqué en échec alors que la sauvegarde figurait bien dans la liste et passait la validation d'intégrité. Le dossier est désormais résolu en **identifiant d'élément** (`items/{id}/children`), dont la pagination fonctionne ; les sous-dossiers sont parcourus par identifiant plutôt que par chemin reconstruit, et le listing ne demande que les champs utiles, 200 par page. Le rattrapage se fait tout seul à la sauvegarde suivante.
- **TEC-249** — **Un échec de sauvegarde indique désormais l'étape en cause.** Le message se limitait au nom de la destination suivi de l'erreur brute, sans dire s'il s'agissait de l'envoi de la sauvegarde, du miroir des PDF ou des pièces jointes ; il traînait en outre le renvoi vers la documentation HTTP ajouté par la bibliothèque réseau. L'étape est nommée et ce suffixe est retiré, dans l'écran des sauvegardes comme dans l'e-mail de notification.
- **TEC-249** — **Les appels à OneDrive sont désormais réessayés quand le service limite le débit ou répond une erreur passagère.** La moindre réponse `429` (trop de requêtes) ou `500/502/503/504` interrompait toute la sauvegarde, alors que Microsoft Graph limite volontiers le débit sur un dossier de plusieurs milliers de fichiers — précisément la situation du rattrapage qui suivra ce correctif. Chaque requête (listing, envoi, envoi par tranches, suppression) est retentée jusqu'à quatre fois : l'application attend le délai indiqué par le serveur dans l'en-tête `Retry-After` quand il en envoie un, sinon 2, 4, 8 puis 16 secondes ; le délai est plafonné à une minute pour qu'une sauvegarde reste bornée dans le temps. Les autres erreurs — un jeton expiré, un dossier absent — ne sont pas retentées : elles n'ont aucune raison de mieux se passer à la seconde tentative.
- **TEC-249** — **Un dossier supprimé sur OneDrive pendant la sauvegarde ne fait plus échouer le miroir.** L'inventaire du contenu distant parcourt les sous-dossiers un à un ; si l'un d'eux disparaissait entre le moment où il était repéré et celui où il était ouvert, l'erreur remontait et interrompait toute la copie — le défaut même que ce lot corrige. Le dossier manquant est désormais traité pour ce qu'il est : il ne contient plus rien. Au pire, son ancien contenu est renvoyé à la sauvegarde suivante.
- **TEC-248** — **Les écrans Factures clients, Factures fournisseurs et Paiements ne montraient pas toujours le résultat d'une action.** Même cause que sur l'écran Documents (TEC-246, corrigée en 1.16.2 et confirmée à l'usage) : l'affichage dépendait d'un **second** appel au serveur lancé après la mutation, et tout ce qui retardait cette requête ou y répondait depuis un état antérieur laissait l'écran inchangé alors que l'enregistrement avait bien eu lieu. Ces écrans s'appuient désormais sur l'objet renvoyé par le serveur : création, modification, duplication, mise en irrécouvrable, restauration, dépôt de pièce jointe, suppression et annulation de règlement se reflètent immédiatement. Une facture créée s'affiche même si les filtres actifs l'excluraient — masquer ce que l'utilisateur vient d'enregistrer est pire que montrer une ligne qu'un filtre aurait écartée. L'enregistrement d'un règlement recharge la **seule facture concernée** plutôt que la liste. Trois cas gardent le rechargement, faute de retour exploitable : l'envoi d'e-mail, l'archivage en masse et le repli en cas d'erreur.
- **BIZ-245** — **Les fichiers Markdown (`.md`) étaient refusés au dépôt**, tout comme `.json`, `.xml` et `.log`. Ces formats n'ont pas de signature binaire propre et la liste blanche par extension ne connaissait que `.csv` et `.txt` — un rapport de clôture rédigé en Markdown se voyait donc opposer « Format de fichier non pris en charge ». Ils sont acceptés dès lors que leur contenu se décode comme du texte, avec le type MIME correspondant.
- **TEC-246** — **L'écran Documents ne montrait pas le document qui venait d'être déposé.** L'affichage dépendait d'un **second** appel au serveur lancé après le dépôt : si cette requête tardait, échouait ou répondait un état antérieur, l'écran restait inchangé alors que l'enregistrement avait bien eu lieu. Or le serveur renvoie déjà le document créé dans sa réponse : la liste s'appuie désormais dessus, et seules les suggestions d'étiquettes sont rechargées. Même principe pour la modification et la suppression. *(Cause non reproduite en local, ni en dev, ni sur le build de production servi par FastAPI ; ce correctif supprime la dépendance plutôt que d'attendre de comprendre pourquoi la seconde requête se comporte mal chez l'utilisateur.)*
- **TEC-243** — **Les téléchargements passant par l'API renvoyaient `{"detail":"Not authenticated"}`.** Ils étaient déclenchés par `window.open`, or le jeton d'accès vit en mémoire et n'est ajouté qu'aux requêtes émises par le client HTTP : une requête ouverte par le navigateur arrivait sans identification. Le défaut préexistait sur les **exports CSV** du bilan et du journal ; les exports PDF et le téléchargement des documents l'avaient hérité. Un utilitaire commun récupère désormais le fichier via le client authentifié et le remet au navigateur, en respectant le nom choisi par le serveur (`bilan_2025.pdf`).
- **TEC-244** — **Les écrans Bilan et Compte de résultat n'avaient pas la même barre d'actions** : le premier regroupait sélecteur d'exercice et exports dans l'en-tête, le second les dispersait dans la zone de filtres, avec des boutons de taille différente. Le compte de résultat adopte la présentation du bilan, et gagne au passage l'export CSV qui lui manquait.
- **TEC-237** — **Le compte de résultat d'un exercice clôturé s'affichait entièrement à zéro**, à l'écran comme à l'export. Le calcul agrège toutes les écritures de l'exercice, or l'écriture de clôture a précisément pour rôle de solder les comptes de charges et de produits : chaque compte ressortait donc nul par construction, et le résultat avec. Le comble : c'est justement après la clôture qu'on veut lire et archiver ce document. Les écritures de clôture sont désormais exclues du compte de résultat (cas réel : 0 € affiché au lieu d'un déficit de 7 444,32 €).
- **TEC-238** — **Le total du passif du bilan s'affichait en négatif.** Les lignes de balance portent un solde en convention débitrice (débit − crédit), qui laisse tout compte de passif négatif : le bilan opposait un actif de 48 219,52 € à un passif de −48 219,52 €. Le passif est désormais présenté en convention créditrice, les deux colonnes se lisent en positif et leurs totaux sont égaux. Sur un exercice clôturé, la ligne « résultat de l'exercice » n'est plus répétée à zéro : le résultat figure déjà en 120000/129000 au passif.
- **TEC-239** — **Les réponses de l'API portent désormais `Cache-Control: no-store`.** Rien ne l'interdisait auparavant à un intermédiaire (heuristique du navigateur, proxy inverse du NAS) de rejouer une réponse antérieure, laissant un écran afficher des montants qui n'existent plus jusqu'au F5. Mesure défensive : des données comptables ne doivent jamais venir d'un cache.
- **TEC-235** — **Un règlement en espèces saisi sur la mauvaise facture ne pouvait plus être annulé.** Un règlement en espèces porte `deposited = True` dès sa création — l'argent est *en caisse* —, et l'éligibilité à l'annulation se basait sur ce seul indicateur : toute saisie en espèces était donc définitivement figée, avec un message affirmant qu'elle « est déjà arrivée sur le compte bancaire », ce qui est faux pour de la caisse. L'annulation reste ouverte tant que l'argent n'a pas atteint la banque : les espèces sont désormais annulables (le mouvement de caisse est supprimé avec le règlement et les soldes sont recalculés), tandis qu'un chèque remis, un virement, un rapprochement bancaire ou un exercice clôturé continuent de bloquer.
...
- **TEC-228** — **Un compte utilisé mais absent du plan comptable déséquilibrait silencieusement l'exercice**. Un tel compte est invisible des deux côtés de la chaîne : la clôture ne le solde pas (elle ne connaît que les comptes déclarés charge/produit) et le report à nouveau ne le reprend pas (comptes actif/passif) — son solde reste suspendu et fausse l'équilibre d'autant. Cas réel : le compte **654000** (pertes sur créances irrécouvrables), écrit par la mise en irrécouvrable mais absent du plan d'une base ancienne, a bloqué une ouverture d'exercice avec 170,50 € d'écart. Les **vérifications avant clôture** signalent désormais ces comptes avec leur solde.
- **TEC-229** — **Le plan comptable se complète au démarrage**. `seed_default_accounts` est idempotent mais n'était appelé qu'à la demande : une base créée avant l'ajout d'un compte au plan par défaut ne le recevait jamais, alors que le moteur écrit dessus. Le complément est désormais appliqué au démarrage de l'application et journalisé.
...
- **BIZ-232** — **Régénérer les écritures comptables d'une facture** (administrateur). Un correctif du moteur ne touche pas les factures déjà enregistrées, et une facture payée n'est plus éditable : il n'existait donc aucun moyen de reconstruire ses écritures depuis l'application. L'action est disponible dans le menu de ligne de l'écran Factures ; elle refuse d'agir sur un exercice clôturé et est tracée dans l'audit.
